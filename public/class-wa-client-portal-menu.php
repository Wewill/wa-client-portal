...
class Wa_Client_Portal_Menu {
    ...
    public static function add_logged_in_menu_items( $items, $args ) {
        ...
        if ( is_user_logged_in() ) {
            ...
            $parent_item .= '<a href="#" data-bs-toggle="collapse" data-bs-target="#accountNavContent" aria-expanded="false" aria-controls="accountNavContent"><i class="bi bi-person-heart fs-4 lh-0"></i></a>';

            // End parent item.
            $parent_item .= '</li>';

            // Store menu content in footer action
            add_action( 'wp_footer', function() use ( $portal_url ) {
                echo '<div class="collapse fixed-top --zi-1000 rounded-start-2" id="accountNavContent">';
                echo '<div class="container-fluid account-nav">';
                echo '<ul class="sub-menu subline"><i class="icon icon-down-right"></i>';
                ...
                if ( !empty( $template_pages ) ) {
                    ...
                    echo sprintf(
                        '<li class="menu-item menu-item-type-custom menu-item-object-custom fs-5 px-2 fw-bold"><a href="%s">%s</a></li>',
                ...
                foreach ( $private_pages as $page ) {
                    ...
                    if ( current_user_can( 'read_private_pages', $page->ID ) )
                        echo sprintf(
                            '<li id="menu-item-client-portal-%1$d" class="menu-item menu-item-type-post_type menu-item-object-page fs-5 px-2 fw-medium"><a href="%2$s">%3$s</a></li>',
                            esc_attr( $page->ID ),
                ...
                // Add logout link.
                echo '<li class="menu-item menu-item-type-custom menu-item-object-custom fs-5 px-2 fw-bold"><a href="' . esc_url( wp_logout_url( home_url() ) ) . '">' . esc_html__( 'Logout', 'wacp' ) . '</a></li>';
