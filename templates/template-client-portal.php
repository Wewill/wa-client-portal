...
while ( have_posts() ) :
	...
	the_post();
	        ...
	        if ( !is_user_logged_in() ) {
				...
				echo '<h6 style="text-align: left;">' . esc_html__( 'Login Required', 'wacpm' ) . '</h6>';
                echo wp_login_form( [ 'echo' => false ] );
				// Display a link to the password recovery page.
				echo '<p><a href="' . wp_lostpassword_url() . '">' . esc_html__( 'Lost your password?', 'wacpm' ) . '</a></p>';
			...
			// Display a register form container 
				...
				echo '<h6 style="text-align: left;">' . esc_html__( 'Register', 'wacpm' ) . '</h6>';
				if ( get_option( 'users_can_register' ) ) {
					echo '<form action="' . esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ) . '" method="post">';
					echo '<p><label for="user_login">' . esc_html__( 'Username', 'wacpm' ) . '</label><input type="text" name="user_login" id="user_login" class="input" required></p>';
					echo '<p><label for="user_email">' . esc_html__( 'Email', 'wacpm' ) . '</label><input type="email" name="user_email" id="user_email" class="input" required></p>';
					do_action( 'register_form' );
					echo '<p><input type="submit" name="wp-submit" id="wp-submit" class="button button-primary" value="' . esc_attr__( 'Register', 'wacpm' ) . '"></p>';
					echo '</form>';
				} else {
					echo '<p>' . esc_html__( 'Registration is currently disabled.', 'wacpm' ) . '</p>';
				}
			...
			echo '<aside class="col-3">';
					...
					if ( ! empty( $private_pages ) ) {
						...
						echo '<p>' . esc_html__( 'No private pages available.', 'wacpm' ) . '</p>';
					}

					// Display a logout link.
					echo '<p><a class="button button-primary" href="' . wp_logout_url( get_permalink() ) . '">' . esc_html__( 'Logout', 'wacpm' ) . '</a></p>';
