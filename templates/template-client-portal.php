	...
	if ( is_email($_POST['magic_email']) ) {
		...
		if ($attempts >= 10) {
			...
			if ( $user && in_array('client-portal', (array)$user->roles) ) {
				...
				// Add centered logo
				$message .= '<div style="text-align:center;margin-bottom:50px;"><a href="www.fifam.fr"><img src="https://www.fifam.fr/wp-content/uploads/2025/11/logotype_fifam_700px.png" alt="Logo" style="max-width:175px;height:auto;"></a></div>';
				...
				$message .= '<p style="color:#888;font-size:10px;margin:0px;">';
				$message .= '<strong>' . esc_html__('Important | Please keep this email carefully:', 'wacp') . '</strong> ';
				. esc_html__('It contains a secret key that allows you to connect securely to your personal FIFAM account space |', 'wacp');
				$message .= '<p>' . esc_html__('Once you are logged in, your access will remain valid for one month |', 'wacp');
				. esc_html__('However, if you change your device (computer, browser, or phone), you can reconnect at any time using this same email |', 'wacp');
				$message .= '<p>' . esc_html__('If you lose this email, simply return to the FIFAM website and enter your email address again to receive a new login link.', 'wacp');
				$message .= '</p>';
				...
				// End 
				$message .= '<p style="color:#888;font-size:10px;margin:0px;">© Festival international du film d\'Amiens, Tous droits réservés.</p>';
